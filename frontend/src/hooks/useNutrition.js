...
export function useNutrition() {
  ...
  const loadMeals = useCallback(async () => {
    try {
      ...
      const [mealsResponse, totalsResponse, targetsResponse] =
        await Promise.all([
      ...
      if (targetsResponse.data.success) {
        ...
        setTargets({
          ...
          macroTargets: targetData.macroTargets || {
            protein: 150,
            carbs: 200,
            fat: 65,
          },
  ...
  const lookupNutrition = useCallback(async (query) => {
    try {
      ...
      const errorMsg =
        err.response?.data?.error || "Failed to lookup nutrition data";
      setError(errorMsg);
  ...
  const deleteMeal = useCallback(
    async (mealId) => {
      try {
        ...
        await loadMeals();
        const errorMsg =
          err.response?.data?.message || err.message || "Failed to delete meal";
        setError(errorMsg);
        throw new Error(errorMsg);
      }
    },
  ...
  const progress = {
    calories: Math.min(
      ((totals.calories || 0) / (targets.baselineCalories || 2000)) * 100,
      100,
    ),
    protein: Math.min(
      ((totals.protein || 0) / (targets.macroTargets?.protein || 150)) * 100,
      100,
    ),
    carbs: Math.min(
      ((totals.carbs || 0) / (targets.macroTargets?.carbs || 200)) * 100,
      100,
    ),
    fat: Math.min(
      ((totals.fat || 0) / (targets.macroTargets?.fat || 65)) * 100,
      100,
    ),
  };
