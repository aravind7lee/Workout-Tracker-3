...
export const useRealTimeDashboard = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      setDashboardStats({
        ...
        weeklyGoal: stats?.weeklyGoal || { completed: 0, target: 4, percentage: 0 },
  ...
  useEffect(() => {
    const handlePlanCreated = (data) => {
      console.log('🚀 INSTANT Dashboard Update - Plan Created:', data.plan.name);
      ...
      setPlanStats(prev => ({
        ...prev,
        totalPlans: prev.totalPlans + 1,
        unsyncedPlans: data.plan.synced ? prev.unsyncedPlans : prev.unsyncedPlans + 1,
    ...
    const handlePlanDeleted = (data) => {
      ...
      setPlanStats(prev => ({
        ...prev,
        totalPlans: Math.max(0, prev.totalPlans - 1),
        syncedPlans: data.plan.synced ? Math.max(0, prev.syncedPlans - 1) : prev.syncedPlans,
        unsyncedPlans: data.plan.synced ? prev.unsyncedPlans : Math.max(0, prev.unsyncedPlans - 1)
      }));
    ...
    const handlePlanSynced = (data) => {
      console.log('☁️ INSTANT Dashboard Update - Plan Synced:', data.realPlan.name);
    ...
    const handleSyncComplete = (data) => {
      console.log('✅ INSTANT Dashboard Update - Sync Complete:', data.planCount, 'plans');
