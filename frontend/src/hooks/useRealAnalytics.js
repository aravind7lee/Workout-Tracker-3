...
export function useRealAnalytics() {
  ...
  const calculateStats = () => {
    ...
    // Calculate XP points
    const xpPoints = history.length * 100 + (currentStreak > 3 ? currentStreak * 50 : 0);
  ...
  const generateAchievements = () => {
    ...
    const achievements = [
      ...
      },
      {
        ...
        unlockedAt: stats.totalWorkouts >= 10 ? new Date(Date.now() - 5 * 24 * 60 * 60 * 1000) : null
      },
      {
        ...
        unlockedAt: stats.totalMeals >= 1 ? new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) : null
      }
    ];
