...
class WorkoutSyncService {
  ...
  getStats() {
    ...
    const todayWorkouts = this.workouts.filter(
      (w) => new Date(w.completedAt).toDateString() === today,
    ).length;

    const weeklyWorkouts = this.workouts.filter(
    ...
    return {
      ...
      todayWorkouts,
      weeklyWorkouts,
      totalCalories: this.workouts.reduce(
        (sum, w) => sum + (w.caloriesBurned || 0),
        0,
      ),
      totalDuration: this.workouts.reduce(
  ...
  broadcastStats() {
    ...
    window.dispatchEvent(
      new CustomEvent("realTimeStatsUpdate", { detail: stats }),
    );
  }
