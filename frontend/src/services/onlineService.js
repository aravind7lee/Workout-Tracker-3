...
class OnlineService {
    ...
    try {
      ...
      if (response.data?.success || response.status === 200) {
        ...
        try {
          ...
          console.warn(
            "Failed to track workout for achievements:",
            achievementError,
          );
        }
  ...
  async saveMeal(mealData) {
    try {
      ...
      try {
        ...
        console.warn(
          "Failed to track meal for achievements:",
          achievementError,
        );
      }
  ...
  async getDetailedAnalytics() {
    try {
      ...
      const [stats, calories, frequency, muscles, achievements] =
        await Promise.all([
  ...
  async trackWorkoutCompletion(workoutData) {
    try {
      ...
      const response = await api.post(
        "/analytics/track-workout-completion",
        workoutData,
      );
      return response.data;
    } catch (error) {
