...
class WorkoutCompletionService {
    ...
    try {
      if (this.isOnline) {
        const response = await fetch(
          `${this.baseURL}/workouts/completed/${userId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
    ...
    try {
      ...
      for (const workout of offlineQueue) {
        try {
          ...
          console.error(
            `❌ Failed to sync workout: ${workout.exercise}`,
            error,
          );
        }
      }
  ...
  triggerRealTimeUpdates(workout) {
    ...
    // Update achievements
    window.dispatchEvent(
      new CustomEvent("achievementCheck", {
        detail: {
          type: "workout_completed",
          data: workout,
        },
  ...
  getWorkoutStats(userId) {
    try {
      ...
      return {
        ...
        monthlyWorkouts: workouts.filter(
          (w) => new Date(w.completedAt) >= thisMonth,
        ).length,
        totalCalories: workouts.reduce(
          (sum, w) => sum + (w.caloriesBurned || 0),
          0,
