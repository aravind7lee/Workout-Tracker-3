...
const MealTrackingCalendar = () => {
  ...
  const getMealCountColor = (count) => {
    ...
    if (count <= 4) return 'bg-red-600/20 text-red-500 border border-red-600/30';
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="flex items-center justify-between gap-2">
          <button
            ...
            className="px-3 py-2 bg-neutral-800/50 hover:bg-neutral-700/50 text-neutral-300 rounded-lg text-sm font-bold transition-colors"
          >
            ←
          </button>
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2">
          {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => (
            <div key={index} className="text-center text-xs font-bold text-neutral-400 py-1">
            ...
            >
              <div className="flex sm:flex-col items-center sm:text-center gap-3 sm:gap-2">
                <div className="flex-shrink-0">
                  <div className={`text-sm font-bold ${day.isToday ? 'text-orange-400' : 'text-neutral-400'}`}>
                    {day.dayName}
                  </div>
                  <div className={`text-lg font-bold ${day.isToday ? 'text-white' : 'text-neutral-300'}`}>
                    {day.date.getDate()}
                  </div>
                </div>
  ...
  const renderDailyView = () => {
    ...
    return (
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="text-center py-4 sm:py-6 bg-neutral-900/30 rounded-lg">
        ...
        {todayMeals.length > 0 ? (
          <div className="space-y-2 sm:space-y-3">
            {todayMeals.map((meal, index) => (
              <div key={index} className="bg-neutral-900/30 rounded-lg p-3 sm:p-4 border border-neutral-800/50">
          ...
          <div className="text-center py-6 sm:py-8">
            ...
            <div className="text-neutral-400 text-sm sm:text-base">No meals logged for this day</div>
            <div className="text-xs text-neutral-500 mt-2">Start tracking your nutrition!</div>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="meal-calendar-container space-y-3 sm:space-y-4">
      ...
      <div className="bg-gradient-to-br from-black/95 to-neutral-900/95 rounded-lg sm:rounded-xl border border-orange-500/20 p-3 sm:p-4 lg:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
          ...
          <div className="flex items-center justify-center gap-1 text-xs text-red-500 bg-red-600/20 px-2 py-1 rounded-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-3 sm:mb-4">
          <div className="text-center p-2 sm:p-3 bg-neutral-900/30 rounded-lg">
            <div className="text-lg sm:text-xl lg:text-2xl font-black text-orange-400 mb-1">{totalMealsToday}</div>
            <div className="text-xs text-neutral-400 uppercase tracking-wide">Today</div>
          </div>
          <div className="text-center p-2 sm:p-3 bg-neutral-900/30 rounded-lg">
            <div className="text-lg sm:text-xl lg:text-2xl font-black text-red-500 mb-1">{weeklyTotal}</div>
            <div className="text-xs text-neutral-400 uppercase tracking-wide">Week</div>
          </div>
          <div className="text-center p-2 sm:p-3 bg-neutral-900/30 rounded-lg">
            <div className="text-lg sm:text-xl lg:text-2xl font-black text-red-500 mb-1">{monthlyTotal}</div>
            <div className="text-xs text-neutral-400 uppercase tracking-wide">Month</div>
          </div>
          <div className="text-center p-2 sm:p-3 bg-neutral-900/30 rounded-lg">
            <div className="text-lg sm:text-xl lg:text-2xl font-black text-red-600 mb-1">{averageMealsPerDay}</div>
            <div className="text-xs text-neutral-400 uppercase tracking-wide">Avg</div>
          </div>
        </div>
      ...
      <div className="bg-neutral-900/50 rounded-lg sm:rounded-xl border border-neutral-800/50 p-3 sm:p-4 lg:p-6">
      ...
      <AnimatePresence>
        {selectedDate && (
          ...
          >
            <motion.div
              className="bg-neutral-900 rounded-lg sm:rounded-xl border border-neutral-800/50 p-4 sm:p-6 max-w-sm sm:max-w-md w-full max-h-[90vh] overflow-y-auto"
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => setSelectedDate(null)}
                  className="w-8 h-8 flex items-center justify-center bg-neutral-800/50 hover:bg-neutral-700/50 rounded-lg text-neutral-400 hover:text-white transition-colors"
                >
                  ✕
                </button>
              </div>
              ...
              {/* Mobile Modal Stats */}
              <div className="text-center py-4 bg-black/50 rounded-lg mb-4">
                <div className="text-2xl sm:text-3xl font-black text-red-500 mb-1">
              ...
              {selectedDate.meals.length > 0 ? (
                <div className="space-y-2 sm:space-y-3">
                  {selectedDate.meals.map((meal, index) => (
                    <div key={index} className="bg-black/30 rounded-lg p-3 border border-neutral-800/30">
                      <div className="flex justify-between items-start gap-3">
                        <div className="flex-1 min-w-0">
                ...
                <div className="text-center py-6">
                  ...
                  <div className="text-neutral-400 text-sm sm:text-base">No meals logged</div>
                  <div className="text-xs text-neutral-500 mt-1">Start tracking your nutrition!</div>
                </div>
              )}
            </motion.div>
          </motion.div>
