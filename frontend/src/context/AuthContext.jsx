...
import {
  migrateGlobalSplitsToUserSpecific,
  isMigrationCompleted,
} from "../utils/migrateSplitsToUserSpecific";
        ...
        if (
          ...
          try {
            ...
            if (initResult.success) {
              ...
              console.warn(
                "⚠️ User workout data initialization failed on startup:",
                initResult.message,
              );
            }
            ...
            if (planInitResult.success) {
              ...
              console.warn(
                "⚠️ User plan data initialization failed on startup:",
                planInitResult.message,
              );
            }
            ...
            if (!isMigrationCompleted()) {
              ...
              if (migrationResult.success) {
                console.log(
                  `✅ Migration completed on startup: ${migrationResult.migratedCount} splits migrated`,
                );
              } else {
                console.warn(
                  "⚠️ Migration failed on startup:",
            ...
            console.log(
              "✅ User-specific workout splits initialized on startup",
            );
          } catch (parseError) {
  ...
  const login = (userData, authToken) => {
    try {
      ...
      if (initResult.success) {
        ...
        console.warn(
          "⚠️ User workout data initialization failed:",
          initResult.message,
        );
      }
      ...
      if (planInitResult.success) {
        ...
        console.warn(
          "⚠️ User plan data initialization failed:",
          planInitResult.message,
        );
      }
      ...
      if (!isMigrationCompleted()) {
        ...
        if (migrationResult.success) {
          console.log(
            `✅ Migration completed: ${migrationResult.migratedCount} splits migrated`,
          );
        } else {
    ...
    try {
      ...
      if (updatedUserData?.email && profileStorage) {
        if (updatedUserData.profileImage && profileStorage.saveProfilePhoto) {
          profileStorage.saveProfilePhoto(
