...
export const cleanUserWorkouts = (currentUser) => {
  ...
  try {
    console.log(`🧹 Cleaning workouts for user: ${currentUser.id || currentUser._id}`);
    ...
    const otherUsersWorkouts = allWorkouts.filter(workout => {
      const hasUserId = workout.userId && 
...
export const clearAllFakeData = () => {
  try {
    ...
    console.log(`✅ Fake data cleanup completed. Kept ${realWorkouts.length} real workouts`);
    return {
      success: true,
