...
export const migrateUserSplits = (user) => {
  try {
    ...
    const userSplits = globalSplits.filter(split => 
      split.userId === userId || 
    ...
    if (userSplits.length > 0) {
      ...
      console.log(`🚀 Migrated ${userSplits.length} splits to user-specific storage for user ${userId}`);
    } else {
...
export const getUserSplits = (user) => {
  try {
    ...
    const userId = user.id || user._id;
    const userSpecificKey = `custom_workout_splits_${userId}`;
    const userSplits = JSON.parse(localStorage.getItem(userSpecificKey) || '[]');
    
    // Double-check ownership
    return userSplits.filter(split => 
      split.userId === userId || 
      split.createdBy === user.name ||
      split.createdBy === user.email
    );
...
export const saveUserSplit = (user, split) => {
  try {
    ...
    const existingSplits = JSON.parse(localStorage.getItem(userSpecificKey) || '[]');
...
export const deleteUserSplit = (user, splitId) => {
  try {
    ...
    const existingSplits = JSON.parse(localStorage.getItem(userSpecificKey) || '[]');
    const updatedSplits = existingSplits.filter(split => split.id !== splitId);
...
export const initializeUserSplits = (user) => {
  try {
    ...
    if (!existingStorage) {
      // Create empty user-specific storage
      localStorage.setItem(userSpecificKey, JSON.stringify([]));
      console.log(`📝 Initialized empty user-specific splits storage for user ${userId}`);
    }
