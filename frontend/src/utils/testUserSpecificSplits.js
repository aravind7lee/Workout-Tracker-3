...
export const testUserSpecificSplits = () => {
  ...
  try {
    ...
    const user1HasUser2Split = user1Splits.some(
      (split) => split.name === "User 2 Split",
    );
    const user2HasUser1Split = user2Splits.some(
      (split) => split.name === "User 1 Split",
    );

    if (!user1HasUser2Split && !user2HasUser1Split) {
      console.log(
        "✅ ISOLATION TEST PASSED: Users can only see their own splits",
      );
    } else {
    ...
    if (user2SplitsAfterUser1Delete.length === 1) {
      console.log(
        "✅ ISOLATION AFTER DELETE TEST PASSED: User 2 splits unaffected",
      );
    } else {
      console.error(
        "❌ ISOLATION AFTER DELETE TEST FAILED: User 2 splits affected",
      );
    }
...
if (process.env.NODE_ENV === "development") {
  console.log(
    "🔧 Development mode detected - user-specific splits utility loaded",
  );
