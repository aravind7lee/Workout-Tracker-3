...
export const migrateGlobalSplitsToUserSpecific = () => {
  try {
    console.log('🔄 Starting migration of global splits to user-specific storage...');
    
    // Get existing global splits
    const globalSplits = JSON.parse(localStorage.getItem('custom_workout_splits') || '[]');
    ...
    if (orphanedSplits.length > 0) {
      ...
      console.log(`📦 Backed up ${orphanedSplits.length} orphaned splits to ${orphanedKey}`);
    }
    
    // Mark migration as complete
    localStorage.setItem('splits_migration_completed', new Date().toISOString());
    
    console.log(`🎉 Migration completed! Migrated ${totalMigrated} splits to user-specific storage`);
...
export const getMigrationStatus = () => {
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('custom_workout_splits_') && key !== 'custom_workout_splits') {
