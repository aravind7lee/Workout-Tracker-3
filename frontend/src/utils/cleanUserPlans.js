...
export const cleanUserPlans = (currentUser) => {
  ...
  try {
    console.log(
      `🧹 Cleaning plans for user: ${currentUser.id || currentUser._id}`,
    );
...
export const clearAllFakePlans = () => {
  try {
    ...
    console.log(
      `✅ Fake plan data cleanup completed. Kept ${realPlans.length} real plans`,
    );
    return {
      success: true,
...
export const initializeUserPlanData = (user) => {
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("userPlanDataInitialized", {
        detail: {
          user,
          cleanupResult,
        },
