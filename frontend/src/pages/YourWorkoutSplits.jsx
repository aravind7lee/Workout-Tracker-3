...
const YourWorkoutSplits = () => {
  ...
  const getSyncStatusDisplay = () => {
    switch (syncStatus) {
      ...
      case 'syncing': return { icon: '🔄', text: 'Loading...', color: 'text-red-500' };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-neutral-900 to-black">
      ...
      <div className="bg-gradient-to-r from-gray-950 via-black to-gray-950 border-b border-red-700/20">
        <div className="max-w-7xl mx-auto px-3 py-3">
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-400">Professional Gym Tracking</p>
                </div>
              </div>
              ...
              <button
                ...
                className="p-1.5 bg-neutral-900/60 text-white rounded-md text-xs"
              >
                {syncStatus === 'syncing' ? '🔄' : '🔄'}
              </button>
            </div>
            ...
            {/* Compact Status Bar */}
            <div className="flex items-center justify-between bg-neutral-900/40 rounded-lg p-2 border border-neutral-800/50">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1">
                  <div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></div>
                  <span className="text-xs font-medium text-red-500">REAL-TIME</span>
                </div>
                
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <div className="text-sm font-bold text-white">{realTimeStats.totalSplits}</div>
                    <div className="text-xs text-neutral-400">Total Plans</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-sm font-bold ${statusDisplay.color}`}>0</div>
                    <div className="text-xs text-neutral-400">Synced</div>
                  </div>
                </div>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        {(customSplits.length > 0 || searchQuery) && (
          <div className="relative mb-4">
            <div className="bg-neutral-900/40 rounded-lg p-2 border border-neutral-800/50">
              <div className="relative">
                <input 
                  value={searchQuery} 
                  onChange={e => setSearchQuery(e.target.value)} 
                  className="w-full p-2 pl-8 pr-8 rounded-md bg-black/60 border border-neutral-700 text-white placeholder-neutral-400 text-sm" 
                  placeholder="Search your plans..." 
                />
                <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 text-neutral-400 w-4 h-4" />
                <div className="absolute right-2 top-1/2 transform -translate-y-1/2 text-neutral-400 text-sm">🔍</div>
                {searchQuery && (
                  <button
                    onClick={() => setSearchQuery('')}
                    className="absolute right-6 top-1/2 transform -translate-y-1/2 text-neutral-400 text-xs p-1"
                  >
                    ✕
                  </button>
                )}
        ...
        {searchQuery && filteredSplits.length === 0 && customSplits.length > 0 ? (
          <div className="text-center py-12 bg-neutral-900/30 rounded-xl border border-neutral-800">
            ...
            <button
              onClick={() => setSearchQuery('')}
              className="bg-red-800 hover:bg-purple-700 text-white px-4 py-2 rounded-lg transition-colors"
            >
              Clear Search
            </button>
          </div>
          ...
          <div className="text-center py-6 bg-neutral-900/30 rounded-lg border border-neutral-800">
            ...
            <p className="text-neutral-400 mb-4 text-sm px-4">
              {isAuthenticated() 
            ...
            {isAuthenticated() ? (
              <Link
                to="/custom-split-builder"
                className="bg-gradient-to-r from-red-800 to-red-700 text-white px-4 py-2 rounded-lg font-medium inline-flex items-center gap-2 text-sm"
              >
                <Plus className="w-3 h-3" />
              ...
              <Link
                to="/login"
                className="bg-red-700 text-white px-4 py-2 rounded-lg font-medium inline-flex items-center gap-2 text-sm"
              >
                <span>🔑</span>
          ...
          <div className="space-y-4">
            {searchQuery && (
              <div className="bg-neutral-900/40 rounded-lg p-3 border border-neutral-800/50">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filteredSplits.map((split) => (
                <motion.div 
                  key={split.id} 
                  className="bg-neutral-900/80 border border-neutral-800/60 rounded-lg overflow-hidden hover:border-red-700/50 transition-all duration-300"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3 }}
                >
                  {/* Compact Header */}
                  <div className="bg-neutral-900/60 p-3 border-b border-neutral-800/50">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-1 mb-1">
                          <span className="text-xs font-medium text-red-600 bg-red-700/20 px-1.5 py-0.5 rounded border border-red-700/30">
                            🔥 Local
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-0.5">
                        <button
                          ...
                          className="text-red-500 p-1 rounded hover:bg-blue-900/20"
                        >
                    ...
                    <div className="flex items-center gap-1 text-xs">
                      <span className="text-neutral-300">{split.difficulty || 'General'}</span>
                  ...
                  <div className="p-3">
                    <div className="space-y-1.5 mb-3">
                      {split.exercises?.slice(0, 2).map((exercise, index) => (
                        <div key={index} className="flex items-center gap-2 bg-neutral-900/40 rounded p-2 border border-neutral-800/30">
                          <span className="text-red-600 font-bold text-xs bg-red-700/20 w-4 h-4 rounded-full flex items-center justify-center">
                            {index + 1}
                          </span>
                          <div className="flex-1 min-w-0">
                            <div className="text-white font-medium text-xs truncate">{exercise.name}</div>
                            <div className="text-neutral-400 text-xs">{exercise.sets}</div>
                          </div>
                        </div>
                      ))}
                    ...
                    {/* Compact Meta */}
                    <div className="bg-neutral-900/40 rounded p-2 border border-neutral-800/30 mb-3">
                    ...
                    <div className="space-y-1.5">
                      <button
                        ...
                        className="w-full bg-red-800/20 text-purple-300 border border-red-700/30 py-2 px-3 rounded text-xs font-medium flex items-center justify-center gap-1"
                      >
                        <span>🏋️</span>
                      ...
                      <button
                        ...
                        className="w-full bg-red-800 text-white py-2 px-3 rounded text-xs font-medium flex items-center justify-center gap-1"
                      >
      ...
      {selectedSplit && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedSplit(null)}>
          <div className="bg-gradient-to-br from-neutral-900 to-black rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-red-700/30 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-white mb-2">{selectedSplit.name}</h2>
                  <p className="text-neutral-300">{selectedSplit.description}</p>
                </div>
                <button onClick={() => setSelectedSplit(null)} className="text-neutral-400 hover:text-white text-2xl">
                  ×
                </button>
              </div>
                ...
                <div className="space-y-3">
                  {selectedSplit.weeklySchedule ? (
                    Object.entries(selectedSplit.weeklySchedule).map(([day, dayContent]) => {
                      ...
                      return (
                        <div key={day} className="bg-neutral-800/20 rounded-lg border border-neutral-700/30 overflow-hidden">
                          <div className="flex items-center justify-between p-3 border-b border-neutral-700/30">
                            <span className="font-semibold text-white text-lg">{day}</span>
                            <div className={`text-xs px-2 py-1 rounded-full ${
                              isRestDay 
                                ? 'bg-gray-600 text-gray-300' 
                                : 'bg-red-700/20 text-purple-300'
                            }`}>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-red-800 to-red-700 hover:from-purple-700 hover:to-blue-700 text-white py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-neutral-800 hover:bg-neutral-700 text-white py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
                >
